...
func _build_environment() -> void:
	...
	env.ambient_light_color = Color(0.80, 0.78, 0.74)
...
static func _leaf_material(tex_name: String, tint: Color = Color.WHITE) -> StandardMaterial3D:
	var key := tex_name + tint.to_html()
	if _leaf_mats.has(key): return _leaf_mats[key]
	var m := StandardMaterial3D.new()
	m.albedo_texture = load(FOLIAGE_DIR + tex_name + ".png")
	m.albedo_color = tint
...
func _pine_parts() -> Array[PropPart]:
	...
	_tier(parts, mat, 4.4, 3.4, 2.0, 3, true, 0.3)
	_tier(parts, mat, 3.4, 3.2, 4.4, 3, true, 0.25, 1)
	_tier(parts, mat, 2.2, 3.4, 6.6, 3, true, 0.15, 2)
	return parts
