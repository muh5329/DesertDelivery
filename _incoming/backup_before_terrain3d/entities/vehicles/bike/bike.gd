class_name Bike
extends Vehicle
## Arcade motorbike (rider + bike). Kinematic, terrain-aligned. Tunables come from its
## VehicleDefinition (data/vehicles/bike.tres); a controller feeds it a ControlIntent per tick.
...
var max_speed := 27.0          # m/s (~97 km/h)
var reverse_speed := 5.0
var accel := 9.5
var brake_decel := 16.0
var drag := 0.55               # linear
var drag_quad := 0.012         # quadratic
var steer_rate_low := 1.9      # rad/s at standstill
var steer_rate_high := 0.75    # rad/s at max speed
var gravity := 22.0
var wheelbase := 1.5
var ceiling := 170.0
...
var altitude := 0.0
var takeoff_speed := 15.0
var flight_max_speed := 46.0
var flight_thrust := 11.0
...
## Read the tunables from the definition (before _ready, or any time).
func apply_definition(d: VehicleDefinition) -> void:
	definition = d
	max_speed = d.max_speed; reverse_speed = d.reverse_speed; accel = d.accel; brake_decel = d.brake_decel
	drag = d.drag; drag_quad = d.drag_quad; steer_rate_low = d.steer_rate_low; steer_rate_high = d.steer_rate_high
	gravity = d.gravity; wheelbase = d.wheelbase; ceiling = d.ceiling
	takeoff_speed = d.takeoff_speed; flight_max_speed = d.flight_max_speed; flight_thrust = d.flight_thrust
...
func _flight(delta: float) -> void:
	...
	if global_position.y > ceiling and vel.y > 0.0:
		vel.y = 0.0
		flight_pitch = minf(flight_pitch, 0.0)
	velocity = vel
	var sink_before := -vel.y
	move_and_slide()
	if global_position.y > ceiling + 1.0:
		global_position.y = ceiling + 1.0
