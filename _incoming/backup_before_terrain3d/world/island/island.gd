...
## The four hubs: id -> [centre, flat pad radius]. The one place these numbers live.
const HUB_TABLE := {
	&"villa_rosa": [Vector2(-188, 2), 22.0],
	&"hilltop_farm": [Vector2(-153, -146), 18.0],
	&"harbour": [Vector2(109, -90), 18.0],
	&"dunes_lookout": [Vector2(96, 73), 9.0],
}
...
func _gen_ground_cover() -> void:
	...
	_scatter_records(_grass_parts(), grass[0], grass[1], 0.0, false)
...
func _gen_town() -> void:
	...
	var ap := _px(1010, 186)
	_at(ap.x, ap.y, func(): _signpost(sink, Vector3(ap.x + 4, _ground(ap.x + 4, ap.y + 5), ap.y + 5), 20, [["HARBOUR", 1.0], ["LIGHTHOUSE", 1.0], ["VILLA ROSA", -1.0]]))
...
func _define_villa(hb_rec: Hub) -> void:
	var cx := hb_rec.centre.x; var cz := hb_rec.centre.y
	_register(&"villa_rosa_office", "Villa Rosa Office", cx + 6.5, cz - 4, Vector3(-1, 0, 0))
	...
	hb_rec.add_wall(Vector2(cx - 22, cz + 14), Vector2(cx - 4, cz + 26), 1.0)
	hb_rec.add_wall(Vector2(cx + 22, cz + 18), Vector2(cx + 36, cz + 30), 1.0)
...
func _build_farm(hb_rec: Hub) -> void:
	...
	_stone_wall(farm, Vector2(cx + 6, cz - 24), Vector2(cx + 40, cz - 24), 0.8)   # the long wall east of the road (tin cans go here)
	_stone_wall(farm, Vector2(cx + 40, cz - 24), Vector2(cx + 36, cz + 14), 0.8)
	for i in range(4):
...
func _define_farm(hb_rec: Hub) -> void:
	var cx := hb_rec.centre.x; var cz := hb_rec.centre.y
	_register(&"hilltop_farm", "Hilltop Farm", cx - 3, cz + 1, Vector3(0, 0, 1))
	hb_rec.ring_pos = db.location_pos(&"hilltop_farm"); hb_rec.ring_facing = Vector3(0, 0, 1)
	hb_rec.add_wall(Vector2(cx + 6, cz - 24), Vector2(cx + 40, cz - 24), 0.8)   # the tin-can wall
	hb_rec.add_wall(Vector2(cx + 40, cz - 24), Vector2(cx + 36, cz + 14), 0.8)
...
func _define_harbour(hb_rec: Hub) -> void:
	var cx := hb_rec.centre.x; var cz := hb_rec.centre.y
	_register(&"harbour_cafe", "Harbour Cafe", cx + 2, cz + 4, Vector3(0, 0, -1))
...
func _define_lookout(hb_rec: Hub) -> void:
	var cx := hb_rec.centre.x; var cz := hb_rec.centre.y
	var g := _ground(cx, cz)
	_register(&"dunes_lookout", "Dunes Lookout", cx + 1, cz - 1, Vector3(1, 0, 0))
