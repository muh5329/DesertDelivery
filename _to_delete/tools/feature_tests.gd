...
## Run: godot --headless --path . -s tools/feature_tests.gd
...
func _physics_process(delta: float) -> bool:
	...
	if bike == null or pl == null: return false
	if sc == null:
	...
	match phase:
		...
		2:
			if pt > 0.3:
				...
				pl.global_position = Vector3(72, 0.3, 114)   # the lake in the badlands
				print("[test] seabed at the lake = %.2f" % tr.height_at(72, 114))
				_next()
		3:
			if pt > 1.5:
				...
				mark = pl.global_position
				main.cam.set_look(deg_to_rad(90), 0.0)   # face west, toward the lake shore
