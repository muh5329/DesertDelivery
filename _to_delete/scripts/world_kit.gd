class_name WorldKit
extends Node3D
## The building blocks every level is made of: sky/light/fog, the sea, rocks, vegetation kits,
## MultiMesh scatter, houses, walls, lamp posts, signposts, props. Nothing here knows the layout
## of a particular island; `Level` extends this and decides where things go.

var terrain: Terrain
var locations: Dictionary = {}     # delivery rings: name -> {pos: Vector3, facing: Vector3}
var hubs: Dictionary = {}          # name -> Hub (geometry other modules build on)
...
## Overridden by the level: name -> [centre: Vector2, pad radius].
func hub_table() -> Dictionary:
	return {}


func hub(name: String) -> Hub:
	return hubs[name]
...
func _register(name: String, x: float, z: float, facing: Vector3) -> void:
	locations[name] = {"pos": Vector3(x, _ground(x, z), z), "facing": facing.normalized()}
